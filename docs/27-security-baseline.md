# 27 — Security-Baseline (Session I)

Baseline-Security-Maßnahmen nach Session I. Siehe auch [26-deploy-hardening.md](26-deploy-hardening.md).

## 1. Trust-Proxy

Fastify läuft hinter NPM. Ohne `trustProxy: true` sieht Fastify die NPM-Container-IP als Client — Rate-Limit wäre wirkungslos (eine IP für alle Welt). Mit trustProxy wertet Fastify `X-Forwarded-For` aus; NPM setzt den Header in [config/npm-proxy-host.md](../config/npm-proxy-host.md).
...
**Follow-up (Session K):** Granulare Rate-Limits für `/trpc/search.*`, `/trpc/report.*`, `/trpc/comment.create` via tRPC-Middleware — Fastify-Route-Level erreicht nur den gesamten `/trpc`-Prefix.
...
- **AI-Moderation** (Ollama-Integration für Comment-Toxicity) — Session K
