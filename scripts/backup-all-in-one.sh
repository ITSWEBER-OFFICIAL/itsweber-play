#!/usr/bin/env bash
# ITSWEBER Play — Backup Script (All-in-One Variante)
# Auf Unraid als Cronjob:
#   0 3 * * * /mnt/user/appdata/itsweber-play-data/scripts/backup-all-in-one.sh
#
# Erwartet einen laufenden Container `itsweber-play` (siehe docs/41-all-in-
# one-unraid.md). Daten liegen unter /mnt/user/appdata/itsweber-play-data/data.

set -euo pipefail

CONTAINER=${CONTAINER:-itsweber-play}
APPDATA=${APPDATA:-/mnt/user/appdata/itsweber-play-data}
BACKUP_ROOT=${BACKUP_ROOT:-/mnt/user/Backup/itsweber-play}
