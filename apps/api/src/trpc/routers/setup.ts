...
async function loadStatus(prisma: PrismaClient) {
  ...
  // Vor dem Fix 2026-04-20: `bypassWithEnv` hat Redirect ausgeschaltet, wenn
  // INITIAL_ADMIN_EMAIL gesetzt war und noch keine User existierten — das
  // führte zu verwirrenden Frischinstallationen (User landete auf `/` statt
  // `/setup`). Jetzt redirectet der Wizard IMMER, bis `setup.complete` einmal
  // sauber durchgelaufen ist. Das `bypassWithEnv`-Flag bleibt im Payload für
  // Debug/Diagnose, ist aber kein Kriterium mehr für `required`.
